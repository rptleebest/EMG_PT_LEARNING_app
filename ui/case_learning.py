...
def _inject_css():
    st.markdown(
        """
        <style>
            /* 본문 두께 400으로 조절하여 시각적 피로도 감소 */
            .case-text-block, .case-bullet { text-align: left !important; word-break: keep-all !important; line-height: 1.6 !important; font-weight: 400; color: #444; }
            ...
            .edu-table td { border: 1px solid #e2e8f0 !important; color: #334155 !important; padding: 10px; text-align: center; font-weight: 400; }
            .edu-table td.left { font-weight: 700 !important; color: #1e293b !important; background-color: #f8fafc !important; text-align: left; }
            
            /* 강조색 채도 조절 (부드러운 주황색/푸른색) */
            ...
            div[role="radiogroup"] > label { width: 100% !important; background-color: #ffffff; border: 1px solid #e2e8f0 !important; border-radius: 8px; padding: 12px; margin-bottom: 8px; display: flex; font-weight: 400; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
        </style>
...
def _render_teaching_result(teaching, diff_dx):
    ...
    for t in teaching.get("integration", []):
        if "🎯" in t or "추정" in t:
            st.markdown(f'<div style="color: #dc2626; font-size: 1.05rem; font-weight: 700; margin-bottom: 8px;">{html_escape(t)}</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="case-text-block" style="margin-bottom: 8px;">{html_escape(t)}</div>', unsafe_allow_html=True)
    
    if diff_dx:
        st.markdown('<div style="border-left: 5px solid #6366f1; background-color: #f5f3ff; padding: 10px 14px; font-weight: 700; font-size: 1rem; color: #4338ca; margin-top: 20px;">⚖️ 감별진단 가이드</div>', unsafe_allow_html=True)
        for item in diff_dx:
    ...
    if selected != "선택 안 함":
        ...
        for g_key, g_title, g_headers, p_func in [
            ...
            if grouped[g_key]:
                ...
                rows = [[k, p_func(_get_value_for_lesion_side(v, raw_side))["amplitude"], p_func(_get_value_for_lesion_side(v, raw_side))["latency"]] if g_key in ["sensory", "motor"] 
                        else [k, p_func(_get_value_for_lesion_side(v, raw_side))["rest"], p_func(_get_value_for_lesion_side(v, raw_side))["volition"]] if g_key=="muscle"
                        else [k, p_func(_get_value_for_lesion_side(v, raw_side))] for k, v in grouped[g_key].items()]
