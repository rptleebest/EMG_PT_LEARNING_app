...
def _render_finding_block(title, findings, side):
    ...
    st.markdown(
        f'<div class="case-section-label" style="font-size:0.92rem; padding: 6px 8px;">{title}</div>',
    ...
    for idx, (item, values) in enumerate(items):
        ...
        if side == "양쪽" or side == "양측":
            ...
            if "감각" in title or "운동" in title:
                if raw_val == "ncs_delayed":
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">진폭: <span class="text-blue">정상 범위</span></div>')
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">잠복기: <span class="text-red" style="font-weight:800;">지연</span></div>')
                elif raw_val == "ncs_reduced":
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">진폭: <span class="text-red" style="font-weight:800;">감소</span></div>')
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">잠복기: <span class="text-blue">정상 범위</span></div>')
                elif raw_val == "ncs_absent":
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">진폭: <span class="text-red" style="font-weight:800;">반응 소실</span></div>')
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">잠복기: <span class="text-red" style="font-weight:800;">반응 소실</span></div>')
                else:
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">진폭: <span class="text-blue">정상 범위</span></div>')
                    lines.append('<div class="finding-subtext" style="font-size:0.82rem; margin-bottom:2px;">잠복기: <span class="text-blue">정상 범위</span></div>')
                ...
                lines.append(f'<div class="finding-subtext" style="font-size:0.82rem;">판독 결과: <span class="{value_color}" style="font-weight:700;">{norm_val}</span></div>')
    ...
    if block_parts:
        st.markdown(
            f'<div class="case-text-block" style="padding: 6px 8px;">{"".join(block_parts)}</div>',
            unsafe_allow_html=True
        )
...
def render_case_list():
    ...
    st.markdown(
        '<div class="subtle" style="font-size:0.84rem; line-height:1.45; word-break:keep-all;">환자의 임상 증상과 근전도 소견을 실시간 비교 분석하여 임상적 판단력을 기릅니다.</div>',
        unsafe_allow_html=True
    )
    ...
    if selected != "선택 안 함":
        ...
        st.markdown(
            f'<div class="case-subtitle-mobile" style="font-size:0.82rem; margin-top:2px;"><span class="label-strong text-blue">연령/성별:</span> <span class="result-value">{patient["age"]}세 / {patient["sex"]}</span> | <span class="label-strong text-blue">병변측:</span> <span class="result-value">{side}</span></div>',
            unsafe_allow_html=True
        )
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        symptoms_html = "".join(
            [f'<div class="case-bullet" style="font-size:0.82rem; margin-bottom:4px;">• {s}</div>' for s in patient.get("symptoms", [])]
        )
        ...
        for sec_name, items in patient.get("physical_exam", {}).items():
            exam_html.append(
                f'<div class="finding-highlight" style="font-size:0.86rem; margin-top:4px; padding-bottom:1px; border-bottom:none; color:#475569;">[{sec_name}]</div>'
            )
            for i in items:
                parts = i.split(":", 1)
                if len(parts) == 2:
                    exam_html.append(
                        f'<div class="case-bullet" style="font-size:0.82rem; margin-bottom:3px;"><span class="label-strong" style="font-size:0.82rem;">{parts[0]}:</span> <span class="result-value" style="font-size:0.82rem;">{parts[1]}</span></div>'
                    )
                else:
        ...
        if teaching.get("emg_reason"):
            is_emg_skipped = (
                "Blink Reflex" in str(teaching["emg_reason"][0])
                or "H-reflex" in str(teaching["emg_reason"][0])
                or "H-반사" in str(teaching["emg_reason"][0])
                or "눈꺼풀" in selected
                or "뇌졸중" in selected
            )
