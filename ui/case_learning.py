...
def format_inline_eng(text):
    """영문을 인라인 괄호로 표기하며, 약어는 무조건 대문자, 나머지는 소문자로 강제 변환"""
    if not text: return ""
    text = str(text)
...
def _render_finding_block(title_kor, findings, side):
    ...
    for item, values in items:
        ...
        if is_emg:
            st.markdown(f'<div class="muscle-title">{format_inline_eng(item)}</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div style="margin-bottom:6px; font-size:0.95rem; font-weight:800; color:#1e293b;">{format_inline_eng(item)}</div>', unsafe_allow_html=True)
        ...
        if side in ["양쪽", "양측"]:
            ...
            elif is_emg: l_txt, r_txt = _get_emg_line_text(raw_left), _get_emg_line_text(raw_right)
            ...
            elif is_emg: st.markdown(_get_emg_line_text(raw_val), unsafe_allow_html=True)
            else:
...
def render_case_list():
    st.markdown('<div class="title-box" style="margin-bottom:16px;"><div class="title-kor">사례 학습 모드</div></div>', unsafe_allow_html=True)
    ...
    if selected != "선택 안 함":
        ...
        for sec_name, items in patient.get("physical_exam", {}).items():
            ...
            for i in items:
                parts = i.split(":", 1)
                if len(parts) == 2: st.markdown(f'<div style="font-size:0.9rem; margin-bottom:4px;"><span style="font-weight:700;">{format_inline_eng(parts[0])}:</span> <span style="color:#334155;">{format_inline_eng(parts[1])}</span></div>', unsafe_allow_html=True)
                else: st.markdown(f'<div style="font-size:0.9rem; color:#334155; margin-bottom:4px;">• {format_inline_eng(i)}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if diff_dx:
            ...
            for idx, d in enumerate(diff_dx):
                ...
                # 🚨 콜론 형식으로 이어서 가독성 개선
                st.markdown(f'<div style="font-size:0.9rem; margin-bottom:4px;"><span style="font-weight:700;">왜 고려하나:</span> <span style="color:#334155;">{format_inline_eng(d.get("why_consider",""))}</span></div>', unsafe_allow_html=True)
                st.markdown(f'<div style="font-size:0.9rem; margin-bottom:4px;"><span style="font-weight:700;">어떻게 구분하나:</span> <span style="color:#334155;">{format_inline_eng(d.get("how_to_differentiate",""))}</span></div>', unsafe_allow_html=True)
                st.markdown(f'<div style="font-size:0.9rem; margin-bottom:4px;"><span style="font-weight:700; color:#15803d;">실전 팁:</span> <span style="color:#15803d; font-weight:500;">{format_inline_eng(d.get("practical_tip",""))}</span></div>', unsafe_allow_html=True)
