...
def _render_mobile_table(headers, rows, table_id, to_eng):
    ...
    <style>
        ...
        @media screen and (max-width: 700px) {{
            ...
            #{table_id} td::before {{ content: attr(data-label); font-weight: 700; color: #64748b; flex: 1; text-align: left; font-size: 0.75rem; }}
        }}
    </style>
    """
    body = ""
...
def _render_interpretation(report):
    ...
    st.markdown('<div style="margin-top: 35px; padding-top: 20px; border-top: 2px dashed #cbd5e1;"><div style="font-weight: 700; font-size: 1.1rem; margin-bottom: 15px;">🔍 검사 결과 단계별 통합 해석</div>', unsafe_allow_html=True)
    ...
    for key, label, c1, c2 in configs:
        ...
        if items:
            ...
            for t in items:
                st.markdown(f'<div style="color: #444; font-size: 0.95rem; line-height: 1.6; margin-bottom: 6px; padding-left: 10px; word-break: keep-all; font-weight: 400;">• {html_escape(t)}</div>', unsafe_allow_html=True)
    ...
    for t in interp.get("integration", []):
        st.markdown(f'<div style="color: #222; font-size: 0.95rem; line-height: 1.6; margin-bottom: 8px; word-break: keep-all; font-weight: 400;">{html_escape(t)}</div>', unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

def render_input_learning():
    st.markdown('<div style="font-size: 1.6rem; font-weight: 700; color: #0f172a; margin-bottom: 1.2rem;">📊 가상 결과표 판독 학습</div>', unsafe_allow_html=True)
    ...
    if selected != "선택 안 함":
        ...
        meta = report["meta"]
        st.markdown(f'<div style="background-color: #f8fafc; border: 1px solid #cbd5e1; border-radius: 8px; padding: 18px; margin-top: 15px; font-weight: 400;"><b>주요 증상:</b><br/>{html_escape(meta.get("chief", ""))}</div>', unsafe_allow_html=True)

        if report.get("sensory_ncs"):
            st.markdown('<div style="font-weight: 700; color:#1e40af; margin-top: 25px; margin-bottom: 8px;">⚡ 감각신경전도검사 (SNAP)</div>', unsafe_allow_html=True)
        ...
        if report.get("motor_ncs"):
            st.markdown('<div style="font-weight: 700; color:#166534; margin-top: 25px; margin-bottom: 8px;">⚡ 운동신경전도검사 (CMAP)</div>', unsafe_allow_html=True)
