...
def _render_tables(report, to_eng):
    if report.get("sensory_ncs"):
        ...
        st.markdown(f'<div style="font-weight: 800; color: #1e3a8a; margin-top: 15px; margin-bottom: 5px;">감각신경전도검사 (SNAP) <span style="color:#ef4444; font-size:0.85em; background:#fef2f2; padding:2px 6px; border-radius:10px;">이상 소견: {abnormal_cnt}개 신경</span></div>', unsafe_allow_html=True)
    ...
    if report.get("motor_ncs"):
        ...
        st.markdown(f'<div style="font-weight: 800; color: #14532d; margin-top: 15px; margin-bottom: 5px;">운동신경전도검사 (CMAP) <span style="color:#ef4444; font-size:0.85em; background:#fef2f2; padding:2px 6px; border-radius:10px;">이상 소견: {abnormal_cnt}개 신경</span></div>', unsafe_allow_html=True)
    ...
    if report.get("late_response"):
        st.markdown('<div style="font-weight: 800; color: #6b21a8; margin-top: 15px; margin-bottom: 5px;">특수 및 반사 검사</div>', unsafe_allow_html=True)
    ...
    if report.get("needle_emg"):
        ...
        st.markdown(f'<div style="font-weight: 800; color: #b45309; margin-top: 15px; margin-bottom: 5px;">침근전도검사 (Needle EMG) <span style="color:#ef4444; font-size:0.85em; background:#fef2f2; padding:2px 6px; border-radius:10px;">이상 소견: {abnormal_cnt}개 근육</span></div>', unsafe_allow_html=True)
...
def _render_interpretation(report):
    ...
    sections = [
        ...
        ("motor", "2단계: 운동신경전도/반사검사 해석", "#10b981", "#ecfdf5"),
        ("emg", "3단계: 침근전도검사 해석", "#d97706", "#fffbeb")
    ]
    for key, title, b_color, bg_color in sections:
        items = interp.get(key, [])
        if not items: continue
        st.markdown(f'<div style="border-left: 4px solid {b_color}; background-color: {bg_color}; padding: 8px 12px; font-weight: 800; color: #1e293b; margin-bottom: 8px;">{html_escape(title)}</div>', unsafe_allow_html=True)
        for item in items: 
    ...
    # 4단계 (결론을 뒤에 배치 및 영어 모드에서도 한글 고정)
    st.markdown('<div style="border-left: 4px solid #dc2626; background-color: #fef2f2; padding: 8px 12px; font-weight: 800; color: #991b1b; margin-bottom: 12px;">4단계: 검사결과 추정 질환</div>', unsafe_allow_html=True)
    ...
    if interp.get("differential"):
        st.markdown('<div style="border-left: 4px solid #9333ea; background-color: #faf5ff; padding: 8px 12px; font-weight: 800; color: #6b21a8; margin-bottom: 8px;">감별진단 가이드</div>', unsafe_allow_html=True)
        for item in interp.get("differential"):
            st.markdown(f'<div style="color: #4c1d95; font-weight:800; font-size: 0.95rem; padding-left: 5px; margin-top: 5px;">{html_escape(item)}</div>', unsafe_allow_html=True)
...
def render_input_learning():
    ...
    st.markdown('<div style="font-weight: 700; color: #1e293b; margin-bottom: 10px;">학습할 가상 결과표 선택</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        clean_html(
            f"""
            <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; margin-bottom: 15px; margin-top: 20px;">
                ...
                <div style="font-size: 0.9rem; color: #334155; margin-top:8px; line-height: 1.5;"><b>주요 증상:</b> {html_escape(meta.get("chief", ""))}</div>
            </div>
            """
        ), unsafe_allow_html=True
    )

    st.markdown('<div style="font-size: 0.85rem; color: #64748b; margin-bottom: 15px;">💡 팁: 아래 표는 병변측(증상 발생 위치)의 주요 검사 결과를 간략히 요약한 것입니다.</div>', unsafe_allow_html=True)
